...
if(!$conec){
	...
	$total = mysql_num_rows(mysql_query("SELECT nume FROM clientes WHERE nume='$ne'"));
if($total==0){
     $in="insert into clientes(nume, rzs, dom, cp, tel, cel, email, obs)"."values('$ne','$nom','$dir','$cpost','$tel','$cel','$mail','$obs')";
	
    $result=mysql_query($in); // puede usarse sin el uso de la variable
	
    echo "Los datos fueron ingresados";
}else{
    echo 'Esta Escuela ya existe, no se ha ingresado ningun dato';
}
